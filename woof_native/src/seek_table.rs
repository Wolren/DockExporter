...
/// [num_entries:u32] [entries × 44 bytes] [name heap]
...
/// Decode seek table from bytes at given offset.
pub fn decode(data: &[u8], offset: usize) -> Result<(Vec<SeekEntry>, usize), WoofError> {
    ...
    for i in 0..num_entries {
        ...
        let _name_offset = u32::from_le_bytes(data[pos + 8..pos + 12].try_into().unwrap());
        let data_offset = u64::from_le_bytes(data[pos + 12..pos + 20].try_into().unwrap());
        let data_size = u64::from_le_bytes(data[pos + 20..pos + 28].try_into().unwrap());
        let raw_size = u64::from_le_bytes(data[pos + 28..pos + 36].try_into().unwrap());
        let hash = u64::from_le_bytes(data[pos + 36..pos + 44].try_into().unwrap());
