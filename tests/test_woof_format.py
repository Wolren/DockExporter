...
class TestPackUnpackV2:
    ...
    def test_no_compress(self, test_entries):
        packed = pack_woof(test_entries, compress=False)
        unpacked = unpack_woof(packed)
        assert unpacked == test_entries
...
class TestRoundtripEdgeCases:
    def test_empty_archive(self):
        packed = pack_woof({}, compress=True)
    ...
    def test_large_binary(self):
        ...
        for compress in [True, False]:
            packed = pack_woof(entries, compress=compress)
            assert unpack_woof(packed) == entries
...
class TestDirectoryPacking:
    def test_pack_from_directory_roundtrip(self, test_entries, temp_dir):
        ...
        packed = pack_woof_from_directory(temp_dir, compress=True)
        unpacked = unpack_woof(packed)
...
class TestSizeSanity:
    def test_compress_smaller(self, test_entries):
        """Compressed should be smaller or equal for compressible data."""
        uncomp = pack_woof(test_entries, compress=False)
        comp = pack_woof(test_entries, compress=True)
...
class TestDeterminism:
    ...
    def test_deterministic(self, test_entries):
        a = pack_woof(test_entries, compress=True)
        b = pack_woof(test_entries, compress=True)
        assert a == b
...
class TestScenarioFidelity:
    ...
    def test_qgs_project_roundtrip(self):
...
class TestRealData:
    ...
    def test_roundtrip(self, real_data_entries):
        if not real_data_entries:
            pytest.skip("No real data found in tests/real_data/")
        packed = pack_woof(real_data_entries, compress=True)
        assert unpack_woof(packed) == real_data_entries

    def test_no_compress(self, real_data_entries):
        if not real_data_entries:
            pytest.skip("No real data found in tests/real_data/")
        packed = pack_woof(real_data_entries, compress=False)
        assert unpack_woof(packed) == real_data_entries
